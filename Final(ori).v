Require Export Framework.
Require Export ProofBigStepWithType.
Require Export ElemLemmas.
Require Export ElemLemmaIntra.
Require Export ElemLemmaReturn.
(* Require Export ElemLemmaCall1.
Require Export ElemLemmaSideEffect. *)
(* Require compat. *)
Require Export step.
Require Export typing_rules.

Import BigStepWithTypes BigStepAnnot.BigStepAnnot BigStep.BigStep Dom Prog.

Definition ValidMethod (p:Program) (m:Method) : Prop :=
  exists c, PROG.defined_Class p c /\ CLASS.defined_Method c m.

Definition step (p:Program) subclass_test : Method -> PC -> option ClassName -> option PC -> Prop := fun m pc tau opc =>
  ValidMethod p m /\ exists i, instructionAt m pc = Some i /\ step p subclass_test m pc i tau opc.
...
Section hyps.
  ...
  Definition Kind := option ClassName.
  Definition Sign : Set :=  sign.

  Definition PM := ValidMethod.

  Inductive P : SignedMethod Method Sign -> Prop :=
    P_def : forall (m:Method) sgn,
      ValidMethod p m ->
      (METHOD.isStatic m = true -> sgn = static_signature p (METHOD.signature m)) ->
      (METHOD.isStatic m = false -> exists k, sgn = virtual_signature p (METHOD.signature m) k) ->
      P (SM _ _ m sgn).

  Section for_all.
    Variable test : METHOD.t -> Sign -> bool.
    
    Definition for_all_P : bool :=
      for_all_methods p 
      (fun m => 
        if METHOD.isStatic m then test m (static_signature p (METHOD.signature m))
          else for_all _ (fun k => test m (virtual_signature p (METHOD.signature m) k)) L.all).
    ...
    Proof.
      ...
      caseeq (METHOD.isStatic m); intros.
  ...
  Definition istate : Type := IntraNormalState.
  Definition rstate : Type := ReturnState.
  ...
  Definition texec : forall m, PM p m -> Sign -> (PC -> L.t) ->
    PC -> Kind -> stacktype -> option stacktype -> Prop :=
    fun m H sgn se pc tau st ost =>
      exists i, texec p subclass_test m sgn (region (cdr m H)) se pc i tau st ost
        /\ instructionAt m pc = Some i.

  Definition compat : Sign -> istate -> stacktype -> Prop := 
    compat_state p.
  Definition compat_res : Sign -> rstate -> Prop :=
    compat_res p.

  Inductive indist : Sign -> stacktype ->
    stacktype -> pbij -> pbij -> istate -> istate -> Prop :=
    indist_def :forall sgn st1 st2 b1 b2 s1 s2 pc1 pc2 h1 h2 l1 l2,
      st_in kobs (newArT p) (ft p) sgn.(lvt) b1 b2 st1 st2 (pc1,h1,s1,l1) (pc2,h2,s2,l2) ->
      indist sgn st1 st2 b1 b2 (pc1,(h1,s1,l1)) (pc2,(h2,s2,l2)).
  ...
  Inductive init_pc (m:Method) : PC -> Prop :=
    init_pc_def : forall bm,
      ...
      init_pc m (BYTECODEMETHOD.firstAddress bm).
