  ...
  Section DEX_typing_rules.   (** Typing rules **)
   ...
   Definition DEX_tcheck (i:DEX_PC) (ins:DEX_Instruction) : bool :=
      match ins, RT i with
        ...
        | DEX_Move _ r rs, rt1 =>
          in_test rs (MapList.VarMap.dom _ in_test r (MapList.VarMap.dom _ 
        ...
        | DEX_VReturn _ r, rt1 =>          
          in_test r (MapList.dVarMap.dom _
          match sgn.(DEX_resType) with
            | None => false
            | Some kv => 
        ...
        | DEX_Ifcmp _ ra rb o, rt1 =>
          in_test ra (MapList.doVarMap.dom _n_test rb (MapList.doVarMap.dom _          match VarMap.get _ rt1 ra, VarMap.get _ rt1 rb with
            | Some ka, Some kb =>
                (selift i (ka U kb)) && 
                (tsub_next i rt1) &&
        ...
        | DEX_Ifz _ r o, rt1 =>
          in_test r (MapList.doVarMap.dom _          match VarMap.get _ rt1 r with
            | Some k => 
                (selift i k) && 
                (tsub_next i rt1) &&
                (tsub_rt rt1 (RT (DEX_OFFSET.jump i o)))
        ...
        | DEX_Ineg r rs, rt1 =>
          in_test rs (MapList.doVarMap.dom _n_test r (MapList.doVarMap.dom _          match VarMap.get _ rt1 rs with
            | Some ks => (tsub_next i (MapList.update rt1 r (L.Simple ((se i) U ks))) )
            | None => false
          end   

        | DEX_Inot r rs, rt1 =>
          in_test rs (MapList.domVarMap.dom __test r (MapList.domVarMap.dom _         match VarMap.get _ rt1 rs with
            | Some ks => (tsub_next i (MapList.update rt1 r (L.Simple ((se i) U ks))) )
            | None => false
          end   

        | DEX_I2b r rs, rt1 =>
          in_test rs (MapList.dom VarMap.dom _test r (MapList.dom VarMap.dom _        match VarMap.get _ rt1 rs with
            | Some ks => (tsub_next i (MapList.update rt1 r (L.Simple ((se i) U ks))) )
            | None => false
          end

        | DEX_I2s r rs, rt1 =>
          in_test rs (MapList.dom rVarMap.dom _est r (MapList.dom rVarMap.dom _       match VarMap.get _ rt1 rs with
            | Some ks => (tsub_next i (MapList.update rt1 r (L.Simple ((se i) U ks))) )
            | None => false
          end   

        | DEX_Ibinop _ r ra rb, rt1 =>
          in_test r (MapList.dom rtVarMap.dom _st ra (MapList.dom rtVarMap.dom _st rb (MapList.dom rtVarMap.dom _      match VarMap.get _ rt1 ra, VarMap.get _ rt1 rb with
            | Some ka, Some kb => 
        ...
        | DEX_IbinopConst _ r rs _, rt1 =>
          in_test rs (MapList.dom rt1VarMap.dom _t r (MapList.dom rt1VarMap.dom _     match VarMap.get _ rt1 rs with
   ...
   Proof.
     ...
     destruct (VarMap.get (RT i) rt) eqn:H2. 
     apply DEX_vReturn with (k_r:=t1) (kv:=t0); auto.
      apply VarMapMapList.get_some_in_dom.
   ...
   Proof.
    ...
    try (flatten; destrVarMap.get _t.get (RT i) rs) eqn:Hrs; try (inversion H2; fail);
    ...
    flatten2; constructor; try (apply in_test_true_In); auto.
     ...
     destruct (MaVarMapet (RT i) rb) eqn:Hrb; try (inversion H2; fail).
     ...
     (* Ifz *)
     flatten.
     destruct (VarMap.get (RT i) r) eqn:Hr; try (inversion H2; fail).
     flatten_bool; replace_selift. inversion H1.
       (* next successor *)
       replace_tsub_next; search_tsub.
