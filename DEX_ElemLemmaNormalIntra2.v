...
(*   destruct (inv_st_in H2) as [Rin]; clear H2; *)
  ...
  destruct Reg_eq_dec with (x:=rn) (y:=rt) as [Hreg | Hreg].
    ...
    rewrite Hget in H7. inversion H7. subst.
    apply not_leql_trans with (k1:=k0); auto.
    apply leql_join_eq with (k1:=(se pc)); auto.
    rewrite Hget' in H17. inversion H17. subst.
    apply not_leql_trans with (k1:=k1); auto.
    apply leql_join_eq with (k1:=(se pc)); auto.
  ...
  destruct Reg_eq_dec with (x:=rn) (y:=rt) as [Hreg | Hreg].
    ...
    constructor 1 with (k:=L.join k (se pc)) (k':=L.join k0 (se pc)); 
      try (rewrite VarMap.get_update1; auto); auto.
    rewrite Hget in H5. inversion H5. subst.
    apply not_leql_trans with (k1:=lvl); auto.
    apply leql_join_eq with (k1:=(se pc)); auto.
